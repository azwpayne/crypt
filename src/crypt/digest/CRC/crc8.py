...
def crc8(  # noqa: PLR0913
  ...
  *,
  ...
  # 处理每个字节
  for byte in data:
    current_byte = reverse_bits(byte) if ref_in else byte

    # 查表计算
    crc = crc_table[crc ^ current_byte]
...
#   # ITU-T G.704标准示例
#   custom_crc = crc8(
#     test_bytes, poly=0xD5, init=0x00, ref_in=False, ref_out=False, xor_out=0x00
#   )
