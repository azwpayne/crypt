...
# @name    : sha3_ke_128.py
# @author  : azwpayne
# @desc    : SHAKE128 eXtendable-Output Function (XOF) implementation
...
SHAKE128 is an XOF (eXtendable-Output Function) based on Keccak[256]
Capacity = 256 bits, Rate = 1344 bits
"""

# ========== 常量和参数 ==========
RATE = 1344  # 比特率 (bits)
CAPACITY = 256  # 容量 (bits)
LANE_SIZE = 64  # 每个 lane 的比特数
STATE_SIZE = 5  # 5x5 的 state
ROUNDS = 24  # Keccak-f 轮数
BLOCK_SIZE = RATE // 8  # 字节块大小 (168 bytes)
...
def pad_message(message):
  """
  SHAKE128 填充 (pad10*1 with domain separator 0x1F)
  返回填充后的字节数组
  """
  rate_bytes = RATE // 8
...
# ========== 主函数 ==========
def shake128(message, output_length):
  """
  SHAKE128 XOF 主函数
...
def shake128_hex(message, output_length):
  """返回十六进制字符串形式的输出"""
  return shake128(message, output_length).hex()
...
def test_shake128():
  ...
  try:
    ...
    print("Testing SHAKE128 implementation:")
    print("=" * 70)

    all_passed = True
    for i, (input_msg, output_len) in enumerate(test_cases, 1):
      result = shake128(input_msg, output_len)

      # 与 pycryptodome 对比
      ref = SHAKE128.new(input_msg).read(output_len)
    ...
    print("pycryptodome not available, skipping tests")
    return True
  else:
    return all_passed
