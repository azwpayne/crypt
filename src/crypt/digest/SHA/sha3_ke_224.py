...
# @name    : sha3_ke_224.py
# @author  : azwpayne
# @desc    : SHAKE224 eXtendable-Output Function (XOF) implementation
"""
SHAKE224 纯函数实现 (非标准扩展)
基于 Keccak[448] 的 XOF 实现
Capacity = 448 bits, Rate = 1152 bits

Note: SHAKE224 is not part of the FIPS 202 standard.
This is a non-standard extension following the same pattern as SHAKE128/256.
"""

# ========== 常量和参数 ==========
RATE = 1152  # 比特率 (bits)
CAPACITY = 448  # 容量 (bits)
LANE_SIZE = 64  # 每个 lane 的比特数
STATE_SIZE = 5  # 5x5 的 state
ROUNDS = 24  # Keccak-f 轮数
BLOCK_SIZE = RATE // 8  # 字节块大小 (144 bytes)
...
def pad_message(message):
  """
  SHAKE224 填充 (pad10*1 with domain separator 0x1F)
  返回填充后的字节数组
  """
  rate_bytes = RATE // 8
...
# ========== 主函数 ==========
def shake224(message, output_length):
  """
  SHAKE224 XOF 主函数 (非标准扩展)
...
def shake224_hex(message, output_length):
  """返回十六进制字符串形式的输出"""
  return shake224(message, output_length).hex()
...
def test_shake224():
  ...
  print("Testing SHAKE224 implementation:")
  print("=" * 70)

  all_passed = True
  for i, (input_msg, output_len) in enumerate(test_cases, 1):
    result = shake224(input_msg, output_len)
  ...
  print("SHAKE224 tests completed!")
  return all_passed
