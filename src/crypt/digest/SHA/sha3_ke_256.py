...
# @name    : sha3_ke_256.py
# @author  : azwpayne
# @desc    : SHAKE256 eXtendable-Output Function (XOF) implementation
...
SHAKE256 is an XOF (eXtendable-Output Function) based on Keccak[512]
...
BLOCK_SIZE = RATE // 8  # 字节块大小 (136 bytes)
...
def pad_message(message):
  """
  SHAKE256 填充 (pad10*1 with domain separator 0x1F)
  返回填充后的字节数组
  """
  rate_bytes = RATE // 8
...
def shake256_hex(message, output_length):
  """返回十六进制字符串形式的输出"""
  return shake256(message, output_length).hex()
...
def test_shake256():
  ...
  try:
    ...
    print("Testing SHAKE256 implementation:")
    print("=" * 70)

    all_passed = True
    for i, (input_msg, output_len) in enumerate(test_cases, 1):
      result = shake256(input_msg, output_len)

      # 与 pycryptodome 对比
      ref = SHAKE256.new(input_msg).read(output_len)
