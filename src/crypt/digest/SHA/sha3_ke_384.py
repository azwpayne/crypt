...
def pad_message(message):
    """
    SHAKE384 填充 (pad10*1 with domain separator 0x1F)
    返回填充后的字节数组
    """
    rate_bytes = RATE // 8
...
# ========== 主函数 ==========
def shake384(message, output_length):
    """
    SHAKE384 XOF 主函数 (非标准扩展)
...
def shake384_hex(message, output_length):
    """返回十六进制字符串形式的输出"""
    return shake384(message, output_length).hex()
...
def test_shake384():
    ...
    print("Testing SHAKE384 implementation:")
    print("=" * 70)

    all_passed = True
    for i, (input_msg, output_len) in enumerate(test_cases, 1):
        result = shake384(input_msg, output_len)
    ...
    print("SHAKE384 tests completed!")
    return all_passed
