...
def blake2b(
  ...
  # For empty message, we still need to process one block of zeros
  msg_len = len(data)
  if msg_len == 0:
    data = b"\x00" * 128
  elif msg_len % 128 != 0:
  ...
  # For the last block, t = total actual bytes (key block + message)
  # For intermediate blocks, t = (i + 1) * 128
...
def blake2s(
  ...
  # t is the cumulative number of actual bytes processed (before padding)
  # For the last block, t = total actual bytes (key block + message)
  # For intermediate blocks, t = (i + 1) * 64
