# SM4 Block Cipher (Chinese National Standard)
# GB/T 32907-2016

FK = [0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC]

CK = [
    0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269,
    0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
    0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249,
    0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
    0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B2229,
    0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
    0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209,
    0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279
]

SBOX = [
    
]

def _tau(a):
    return (SBOX[(a >> 24) & 0xFF] << 24) | (SBOX[(a >> 16) & 0xFF] << 16) |            (SBOX[(a >> 8) & 0xFF] << 8) | SBOX[a & 0xFF]

def _l(b):
    return b ^ ((b << 2) | (b >> 30)) ^ ((b << 10) | (b >> 22)) ^ ((b << 18) | (b >> 14)) ^ ((b << 24) | (b >> 8))

def _f(x0, x1, x2, x3, rk):
    return x0 ^ _l(_tau(x1 ^ x2 ^ x3 ^ rk))

def _key_expand(key):
    K = [int.from_bytes(key[i:i+4], 'big') ^ FK[i] for i in range(4)]
    rk = []
    for i in range(32):
        K.append(K[i] ^ _l(_tau(K[i+1] ^ K[i+2] ^ K[i+3] ^ CK[i])))
        rk.append(K[i+4])
    return rk

def sm4_encrypt(block, key):
    rk = _key_expand(key)
    X = [int.from_bytes(block[i:i+4], 'big') for i in range(4)]
    for i in range(32):
        X.append(_f(X[i], X[i+1], X[i+2], X[i+3], rk[i]))
    return b''.join(X[35-i].to_bytes(4, 'big') for i in range(4))

def sm4_decrypt(block, key):
    rk = _key_expand(key)
    X = [int.from_bytes(block[i:i+4], 'big') for i in range(4)]
    for i in range(32):
        X.append(_f(X[i], X[i+1], X[i+2], X[i+3], rk[31-i]))
    return b''.join(X[35-i].to_bytes(4, 'big') for i in range(4))
