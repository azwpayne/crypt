...
# @desc    : Blowfish block cipher implementation (16-round Feistel cipher)

"""
Blowfish Block Cipher Implementation

Blowfish is a 16-round Feistel cipher with:
- 64-bit block size
- Variable key length: 32-448 bits (4-56 bytes)
- 16 rounds of encryption
- Key-dependent S-boxes and P-array

Reference: https://www.schneier.com/academic/archives/1994/09/the_blowfish_encrypt.html
"""
...
# Initial S-boxes (hex digits of pi)
INITIAL_S0 = [
  0xD1310BA6,
  0x98DFB5AC,
  0x2FFD72DB,
...
class Blowfish:
  ...
  Blowfish is a 16-round Feistel cipher with a 64-bit block size and
  variable key length (32-448 bits).

  Attributes:
      p_array: The 18 32-bit subkeys (P-array)
  ...
  def __init__(self, key: bytes) -> None:
    ...
    Args:
        key: The encryption key (4-56 bytes, 32-448 bits)

    Raises:
        ValueError: If key length is invalid
    """
    if not 4 <= len(key) <= 56:
      msg = f"Key must be 4-56 bytes, got {len(key)}"
      raise ValueError(msg)

    # Initialize P-array and S-boxes with digits of pi
    self.p_array = INITIAL_P.copy()
    self.s_boxes = [
      INITIAL_S0.copy(),
      INITIAL_S1.copy(),
      INITIAL_S2.copy(),
  ...
  def _key_schedule(self, key: bytes) -> None:
    ...
    This performs the key expansion by XORing the key with the P-array,
    then encrypting all-zero blocks to generate the final subkeys.
    ...
    for i in range(18):
      word = 0
      for j in range(4):
        word = (word << 8) | key[(i * 4 + j) % key_len]
      self.p_array[i] ^= word

    # Encrypt all-zero blocks to generate subkeys
    left, right = 0, 0
    ...
    # Encrypt to generate S-box entries
    for i in range(4):
  ...
  def _f_function(self, x: int) -> int:
    ...
    The F-function splits the 32-bit input into 4 bytes,
    looks them up in the S-boxes, and combines the results.
    ...
    result = (self.s_boxes[0][a] + self.s_boxes[1][b]) & 0xFFFFFFFF
    result ^= self.s_boxes[2][c]
    return (result + self.s_boxes[3][d]) & 0xFFFFFFFF
  ...
  def _decrypt_round(self, left: int, right: int) -> tuple[int, int]:
    ...
    # Undo the swap that was done at the end of encryption
    left, right = right, left

    # Reverse the 16 rounds
...
def encrypt_ecb(key: bytes, plaintext: bytes) -> bytes:
  ...
  Args:
      key: Encryption key (4-56 bytes)
      plaintext: Data to encrypt (will be padded to 8-byte boundary)
...
def decrypt_ecb(key: bytes, ciphertext: bytes) -> bytes:
  """Decrypt data using Blowfish in ECB mode.
  ...
  Raises:
      ValueError: If ciphertext length is not a multiple of 8
  """
