...
class DES3:
    ...
    def __init__(self, key: bytes) -> None:
        ...
        if len(key) not in (16, 24):
            raise ValueError(f"Key must be 16 or 24 bytes, got {len(key)}")
    ...
    def _ede_encrypt_block(self, block: int) -> int:
        ...
        result = _des_block_encrypt(result, self.des3.subkeys)
        return result
    ...
    def _ede_decrypt_block(self, block: int) -> int:
        ...
        result = _des_block_decrypt(result, self.des1.subkeys)
        return result
