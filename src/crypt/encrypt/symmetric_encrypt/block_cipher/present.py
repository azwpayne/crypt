...
def key_schedule_80(key: bytes) -> list[int]:
    ...
    """
    if len(key) != 10:
        raise ValueError(f"Key must be 10 bytes for 80-bit mode, got {len(key)}")
...
def key_schedule_128(key: bytes) -> list[int]:
    ...
    """
    if len(key) != 16:
        raise ValueError(f"Key must be 16 bytes for 128-bit mode, got {len(key)}")
...
def key_schedule(key: bytes) -> list[int]:
    ...
    if len(key) == 10:
        ...
        raise ValueError(f"Key must be 10 or 16 bytes, got {len(key)}")
