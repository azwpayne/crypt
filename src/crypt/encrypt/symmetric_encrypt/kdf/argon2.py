...
class _Argon2Block:
    ...
    def xor(self, other: '_Argon2Block') -> '_Argon2Block':
        """XOR two blocks."""
        result = _Argon2Block()
        result.v = [a ^ b for a, b in zip(self.v, other.v)]
        return result
...
def _compress_block(x: _Argon2Block, y: _Argon2Block) -> _Argon2Block:
    ...
    # Row rounds
    for i in range(4):
        q[i], q[i+1], q[i+2], q[i+3] = _g_function(
...
def _init_memory(
    ...
    for lane_index in range(parallelism):
        lane = []
        for i in range(memory_blocks // parallelism):
            lane.append(_Argon2Block())
        lanes.append(lane)

    # Compute H0
    h0_input = struct.pack("<IIIIII",
        parallelism,
        len(password),
        len(salt),
        len(key),
...
def argon2i(
    ...
    if memory_cost < 8 * parallelism:
        raise ValueError(f"Memory cost must be at least 8*p = {8*parallelism}")
    if time_cost < 1:
    ...
    if memory_blocks < 2 * parallelism:
        memory_blocks = 2 * parallelism
