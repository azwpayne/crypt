...
def _salsa20_8_core(input_bytes: bytes) -> bytes:
    ...
    # Add original values
    output = struct.pack("<16I", *[(x[i] + orig_x[i]) & 0xFFFFFFFF for i in range(16)])
    return output
...
def _blockmix(input_bytes: bytes, r: int) -> bytes:
    ...
    for i in range(2 * r):
        ...
        x = _salsa20_8_core(bytes(a ^ b for a, b in zip(x, block)))
        y.extend(x)
...
def scrypt(
    ...
    if n >= 2**(128 * r // 8):
        raise ValueError("N is too large for the given r")
    ...
    # Step 3: Final PBKDF2
    result = hashlib.pbkdf2_hmac('sha256', password, b, 1, dklen)
