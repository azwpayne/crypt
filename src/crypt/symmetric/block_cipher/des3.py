...
from crypt.symmetric.block_cipher.des import DES, _bytes_to_int, _int_to_bytes, _pkcs7_pad, _pkcs7_unpad
...
def _s_box_substitution(block: int) -> int:
  ...
  from crypt.symmetric.block_cipher.des import S_BOXES
...
def _feistel_function(right: int, subkey: int) -> int:
  ...
  """
  from crypt.symmetric.block_cipher.des import E_TABLE, P_TABLE
...
def _des_block_encrypt(block: int, subkeys: list[int]) -> int:
  ...
  from crypt.symmetric.block_cipher.des import FP_TABLE, IP_TABLE
