...
class TestPBKDF2:
  ...
  def test_rfc6070_vectors(  # noqa: PLR0913
    self, password, salt, iterations, dklen, hash_name, expected
  ):
  ...
  def test_unicode_password(self):
    ...
    password = "пароль"  # noqa: S105 # Russian for "password"
    salt = b"salt"
    iterations = 1000
    dklen = 32
    hash_name = "sha256"
  ...
  def test_unicode_salt(self):
    ...
    result = pbkdf2(password, salt, iterations, dklen, hash_name)
    expected = hashlib.pbkdf2_hmac(
      hash_name, password, salt.encode("utf-8"), iterations, dklen
    )
    assert result == expected
