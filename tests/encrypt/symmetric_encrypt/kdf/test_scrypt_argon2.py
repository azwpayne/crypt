...
from src.crypt.encrypt.symmetric_encrypt.kdf.argon2 import _Argon2Block, argon2, argon2i
from src.crypt.encrypt.symmetric_encrypt.kdf.scrypt import (
  _blockmix,
  _salsa20_8_core,
  scrypt,
)
...
class TestScrypt:
  ...
  def test_scrypt_basic(self):
    ...
    result = scrypt(password=b"password", salt=b"salt", n=2, r=1, p=1, dklen=32)
    assert len(result) == 32
    assert isinstance(result, bytes)

  def test_scrypt_with_strings(self):
    """Test scrypt with string inputs."""
    result = scrypt(password="password", salt="salt", n=2, r=1, p=1, dklen=32)
    assert len(result) == 32
  ...
  def test_scrypt_compare_with_hashlib(self):
    ...
    try:
      expected = hashlib.scrypt(b"password", salt=b"salt", n=2, r=1, p=1, dklen=32)
...
class TestArgon2:
  ...
  def test_argon2i_invalid_memory_cost(self):
    ...
    with pytest.raises(ValueError, match="at least"):
      argon2i(
        b"password", b"somesalt12345678", memory_cost=1, time_cost=1, parallelism=4
      )
  ...
  def test_argon2i_invalid_time_cost(self):
    ...
    with pytest.raises(ValueError, match="at least 1"):
      argon2i(
        b"password", b"somesalt12345678", memory_cost=32, time_cost=0, parallelism=1
      )
  ...
  def test_argon2i_invalid_parallelism(self):
    ...
    with pytest.raises(ValueError, match="at least 1"):
      argon2i(
        b"password", b"somesalt12345678", memory_cost=32, time_cost=1, parallelism=0
      )
