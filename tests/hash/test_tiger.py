...
class TestTigerPadding:
  ...
  def test_tiger_pad_message_short(self):
    ...
    msg = b"abc"
    result = tiger.pad_message_tiger(msg)
  ...
  def test_tiger2_pad_message_short(self):
    ...
    msg = b"abc"
    result = tiger.pad_message_tiger2(msg)
...
class TestTigerInternal:
  ...
  def test_round_function(self):
    ...
    new_a, new_b, new_c = tiger.round_func(a, b, c, x, mult)
  ...
  def test_compress_block(self):
    ...
    a, b, c = tiger.INITIAL_A, tiger.INITIAL_B, tiger.INITIAL_C
    block = b"\x00" * 64

    new_a, new_b, new_c = tiger.compress_block(a, b, c, block)
  ...
  def test_key_schedule(self):
    ...
    schedule = tiger.key_schedule(words)
  ...
  def test_s_boxes_exist_and_correct_size(self):
    ...
    assert len(tiger.S0) == 256
    assert len(tiger.S1) == 256
    assert len(tiger.S2) == 256
    assert len(tiger.S3) == 256

    # All values should be 64-bit
    for box in (tiger.S0, tiger.S1, tiger.S2, tiger.S3):
