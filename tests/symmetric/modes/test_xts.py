...
class TestXTSStandalone:
  def test_standalone_test_function(self):
    from crypt.symmetric.modes.xts import test_xts_mode
...
class TestXTSInternalBranches:
  def test_init_with_expanded_key_and_nr(self):
    from crypt.symmetric.block_cipher.aes import key_expansion
...
class TestXTSModeAgainstPyCryptodome:
  ...
  def test_xts_mode_against_pycryptodome_basic(self):
    ...
    # Our implementation
    xts = XTSMode(key=key)
    xts.encrypt(plaintext, tweak.to_bytes(16, "big"))

    # PyCryptodome
    cipher = AES.new(key, AES.MODE_XTS)  # type: ignore[reportAttributeAccessIssue]
    cipher.encrypt(plaintext, tweak=tweak.to_bytes(16, "little"))
